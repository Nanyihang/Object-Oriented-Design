...
namespace CodeAnalysis
{
  ...
  public:
	...
	std::map<int, int> get_function_row();
	std::map<int, int> get_class_row();
  private:
    std::list<element> stack;
    element dbElement;
	std::map <int,int> class_rowss;
	std::map <int,int> function_rows;
  };
  ...
  template<typename element>
  void ScopeStack<element>::TreeWalk(element* pItem, bool details = false)
  {
    ...
    //std::cout << out.str();
	//if(pItem)
	//std::cout << pItem->type_ << pItem->startLineCount_ << pItem->endLineCount_ << std::endl;
	if (pItem->type_ == "class")
	{
		class_rowss[int(pItem->startLineCount_)] = int(pItem->endLineCount_);
	}
	if (pItem->type_ == "function")
	{
		function_rows[int(pItem->startLineCount_)] = int(pItem->endLineCount_);
	}
  ...
  template<typename element>
  std::map<int, int> ScopeStack<element>::get_function_row()
  {
	  return function_rows;
  }
  template<typename element>
  std::map<int, int> ScopeStack<element>::get_class_row()
  {
	  return class_rowss;
  }
